...
def forgot_password():
    if request.method == 'POST':
        email = request.form.get('email')
        print(f"Password reset requested for: {email}")  # Debug log
        ...
        if user:
            ...
            print(f"Generated reset token: {reset_token}")  # Debug log
            
            # Send reset email using the correct function
            try:
                if send_password_reset_email(email, reset_token):
                    ...
                    db.session.rollback()
                    print("Failed to send password reset email")  # Debug log
                    return render_template('auth/forgot_password.html', 
                                         error='Failed to send reset email. Please try again.')
            except Exception as e:
                db.session.rollback()
                print(f"Error in forgot_password route: {str(e)}")  # Debug log
                return render_template('auth/forgot_password.html', 
...
def reset_password():
    ...
    if (session.get('reset_token') != token or 
        not reset_expires or 
        datetime.fromisoformat(reset_expires) < datetime.utcnow()):
        return render_template('auth/reset_password.html', 
    ...
    # For GET request, show the reset form
    return render_template('auth/reset_password.html')
